...
router.get('/', (req, res, next) => {
	Restaurant.find({}, (error, restaurantsFromDB) => {
		if (error) {
			...
			res.render('restaurants/index', { restaurants: restaurantsFromDB, user: req.user })
		}
	})
})
...
router.get('/:restaurant_id/edit', (req, res, next) => {
	Restaurant.findById(req.params.restaurant_id, (error, restaurant) => {
		if (error) {
			...
			res.render('restaurants/update', { restaurant, user: req.user })
		}
	})
})
...
router.get('/:restaurant_id', (req, res, next) => {
	Restaurant.findById(req.params.restaurant_id, (error, restaurant) => {
		if (error) {
			...
			res.render('restaurants/show', { restaurant: restaurant, user: req.user })
		}
	})
})
